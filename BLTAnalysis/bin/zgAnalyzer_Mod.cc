...
void zgAnalyzer::Begin(TTree *tree)
{
    ...
    //
    if(debugFlag)
	cout << "--------Create Branches on tree" << endl;
    // event data
    ...
    //outTree->Branch("dileptonP4", &dileptonP4);
    
    outTree->Branch("leptonOneM", &leptonOneM);
    outTree->Branch("leptonOneE", &leptonOneE);
    outTree->Branch("leptonOneEt", &leptonOneEt);
    ...
    outTree->Branch("leptonTwoM", &leptonTwoM);
    outTree->Branch("leptonTwoE", &leptonTwoE);
    outTree->Branch("leptonTwoEt", &leptonTwoEt);
    ...
    outTree->Branch("photonOneM", &photonOneM);
    ...
    //outGenTree->Branch("genNeutrinoP4", &genNeutrinoP4);
    ...
    outTree->Branch("genLeptonTwoM", &genLeptonTwoM);
    ...
    /////////////////////////////////////////////////////////////////
    outGenTree->Branch("genLeptonOnePt", &genLeptonOnePt);
    outGenTree->Branch("genLeptonOneM", &genLeptonOneM);
    outGenTree->Branch("genLeptonOneEta", &genLeptonOneEta);
    outGenTree->Branch("genLeptonOnePhi", &genLeptonOnePhi);
    outGenTree->Branch("genLeptonOneId", &genLeptonOneId);

    outGenTree->Branch("genLeptonTwoPt", &genLeptonTwoPt);
    outGenTree->Branch("genLeptonTwoM", &genLeptonTwoM);
    outGenTree->Branch("genLeptonTwoEta", &genLeptonTwoEta);
    outGenTree->Branch("genLeptonTwoPhi", &genLeptonTwoPhi);
    outGenTree->Branch("genLeptonTwoId", &genLeptonTwoId);

    outGenTree->Branch("genPhotonOnePt", &genPhotonOnePt);
    outGenTree->Branch("genPhotonOneEta", &genPhotonOneEta);
    outGenTree->Branch("genPhotonOnePhi", &genPhotonOnePhi);

    outGenTree->Branch("genDileptonPt" , &genDileptonPt);
    outGenTree->Branch("genDileptonM"  , &genDileptonM);
    outGenTree->Branch("genDileptonEta", &genDileptonEta);
    outGenTree->Branch("genDileptonPhi", &genDileptonPhi);

    outGenTree->Branch("genDilepgmPt" , &genDilepgmPt);
    outGenTree->Branch("genDilepgmM"  , &genDilepgmM);
    outGenTree->Branch("genDilepgmEta", &genDilepgmEta);
    outGenTree->Branch("genDilepgmPhi", &genDilepgmPhi);
    /////////////////////////////////////////////////////////////////
    //
    ...
    // dilepton
    outTree->Branch("dileptonE", &dileptonE);
    ...
    // dilepgm
    outTree->Branch("dilepgmE", &dilepgmE);
    outTree->Branch("dilepgmPt", &dilepgmPt);
    outTree->Branch("dilepgmEta", &dilepgmEta);
    outTree->Branch("dilepgmPhi", &dilepgmPhi);
    outTree->Branch("dilepgmM", &dilepgmM);
    outTree->Branch("dilepgmDEta", &dilepgmDEta);
    outTree->Branch("dilepgmDPhi", &dilepgmDPhi);
    outTree->Branch("dilepgmDR", &dilepgmDR);
    outTree->Branch("dilepgmMKin", &dilepgmMKin);
    // dilepton vertices
    ...
    string outHistNegName = params->get_output_treename("TotalNegEvents");
    hTotalNegEvents = new TH1D(outHistNegName.c_str(),"TotalNegEvents",30,0.5,30.5);
...
Bool_t zgAnalyzer::Process(Long64_t entry)
{
    ...
    if (!isData) 
		...
		hTotalNegEvents->Fill(eventStep);
    ...
    // Generator objects // 
    ///////////////////////
    TLorentzVector genLeptonOneP4, genLeptonTwoP4, genPhotonOneP4;
    ...
    fLep = false;
    nLepton = 0;
    bool hasValidGenPhoton    = false;
    bool hasValidGenLeptonOne = false;
    bool hasValidGenLeptonTwo = false;
    if(debugFlag)
	cout << "-----Gen start" << endl;
    if (!isData) {
        unsigned count = 0;             
		...
		if(particle->parent >0){	
			if(particle->pdgId == 13){
                		TGenParticle* mother = (TGenParticle*) fGenParticleArr->At(particle->parent);
				if(fabs(mother->pdgId) == 23){
					if(particle->pt > PreSel_pt_mu && fabs(particle->eta) < PreSel_eta_ph){
						genLeptonOneP4.SetPtEtaPhiM(particle->pt, particle->eta, particle->phi, particle->mass);
						hasValidGenLeptonOne = true;
					}
				}
			}
			else if(particle->pdgId == -13){
                		TGenParticle* mother = (TGenParticle*) fGenParticleArr->At(particle->parent);
				if(fabs(mother->pdgId) == 23){
					if(particle->pt > PreSel_pt_mu && fabs(particle->eta) < PreSel_eta_mu){
						genLeptonTwoP4.SetPtEtaPhiM(particle->pt, particle->eta, particle->phi, particle->mass);
						hasValidGenLeptonTwo = true;
					}
				}
			}
			if(fabs(particle->pdgId) == 22){
				//if(particle->pt > PreSel_pt_ph && fabs(particle->eta) < PreSel_eta_ph){
				//	genPhotons.push_back(particle);
				//	genPhotonOneP4.SetPtEtaPhiM(particle->pt, particle->eta, particle->phi, particle->mass);
				//	hasValidGenPhoton = true;
				//}
				genPhotons.push_back(particle);
				genPhotonOneP4.SetPtEtaPhiM(particle->pt, particle->eta, particle->phi, particle->mass);
				hasValidGenPhoton = true;
	...
	if(hasValidGenPhoton && hasValidGenLeptonOne && hasValidGenLeptonTwo){
		genLeptonOneE  = genLeptonOneP4.E();
		genLeptonOnePt  = genLeptonOneP4.Pt();
		genLeptonOneEta = genLeptonOneP4.Eta();
		genLeptonOnePhi = genLeptonOneP4.Phi();
		genLeptonOneM   = genLeptonOneP4.M();

		genLeptonTwoE  = genLeptonTwoP4.E();
		genLeptonTwoPt  = genLeptonTwoP4.Pt();
		genLeptonTwoEta = genLeptonTwoP4.Eta();
		genLeptonTwoPhi = genLeptonTwoP4.Phi();
		genLeptonTwoM   = genLeptonTwoP4.M();

		genPhotonOneE  = genPhotonOneP4.E();
		genPhotonOnePt  = genPhotonOneP4.Pt();
		genPhotonOneEta = genPhotonOneP4.Eta();
		genPhotonOnePhi = genPhotonOneP4.Phi();
		genPhotonOneM = genPhotonOneP4.M();


		genDileptonE  = ( genLeptonOneP4 + genLeptonTwoP4 ).E();
		genDileptonPt  = ( genLeptonOneP4 + genLeptonTwoP4 ).Pt();
		genDileptonM   = ( genLeptonOneP4 + genLeptonTwoP4 ).M();
		genDileptonEta = ( genLeptonOneP4 + genLeptonTwoP4 ).Eta();
		genDileptonPhi = ( genLeptonOneP4 + genLeptonTwoP4 ).Phi();

		genDilepgmE  = ( genLeptonOneP4 + genLeptonTwoP4 + genPhotonOneP4).E();
		genDilepgmPt  = ( genLeptonOneP4 + genLeptonTwoP4 + genPhotonOneP4).Pt();
		genDilepgmM   = ( genLeptonOneP4 + genLeptonTwoP4 + genPhotonOneP4).M();
		genDilepgmEta = ( genLeptonOneP4 + genLeptonTwoP4 + genPhotonOneP4).Eta();
		genDilepgmPhi = ( genLeptonOneP4 + genLeptonTwoP4 + genPhotonOneP4).Phi();
		outGenTree->Fill();
	}
	//outBugTree->Fill();
	if(LeptonMom){
    ...
    for (int i=0; i<fElectronArr->GetEntries(); i++) {
		...
		&& fabs(electron->eta) < PreSel_eta_el
	) {
    ...
    for (int i=0; i<fPhotonArr->GetEntries(); i++) {
		...
		&& fabs(photon->scEta) < PreSel_eta_ph
	    ...
	    photonOneSieie = photon->sieie;
	    photonOneHoverE = photon->hovere;
	    photonOneIneu = photon->neuHadIso;
	    photonOneIph = photon->gammaIso;
	    photonOneIch = photon->chHadIso;
	...
	if(debugFlag){
		cout << "--------ENTER\n";
		cout << "||M:: " << nMuons << "|| EL:: "<< nElectrons << "|| T:: "<< nTaus << "|| Ph:: "<< nPhotons << "|| Jets:: "<< jets.size() << "||\n";
	}
	...
	if(muons.size() == 2) MuFlag = true;
	else if(electrons.size() == 2) ElFlag = true;
	else return kTRUE;
	...
	if(MuFlag){
		...
		for(unsigned int i = 0; i < muons.size(); i++){
			...
			for(unsigned int j = i+1; j< muons.size(); j++){
				...
				tempLepTwoP4.SetPtEtaPhiM(muons[j]->pt,muons[j]->eta, muons[j]->phi,MUON_MASS);
				tempDILEP = tempLepOneP4 + tempLepTwoP4;
				if(
					//tempDILEP.M() < 100 && tempDILEP.M() > 80
					muons[j]->q == -muons[muonOneIndex]->q
					&& tempDILEP.M() > 50
					//&& tempDILEP.M() < 110 				
				){
					hasValidPair = true;
					muonTwoIndex = j;
					//cout << tempDILEP.M() << endl;
					leptonOneP4.SetPtEtaPhiM(muons[muonOneIndex]->pt, muons[muonOneIndex]->eta, muons[muonOneIndex]->phi, MUON_MASS);
					leptonTwoP4.SetPtEtaPhiM(muons[muonTwoIndex]->pt, muons[muonTwoIndex]->eta, muons[muonTwoIndex]->phi, MUON_MASS);
					//cout << (leptonOneP4 + leptonTwoP4).M();//cout << "never" << endl;
				}
			}
			
			//if(hasValidMuon) break;
		}
		if( !hasValidPair)
			return kTRUE;
		eventStep++;
		...
		for(unsigned int i =0; i < electrons.size(); i++){
			...
			for(unsigned int j = i+1; j< electrons.size(); j++){
				TLorentzVector tempLepOneP4,tempLepTwoP4, tempDILEP;
				tempLepOneP4.SetPtEtaPhiM(electrons[electronOneIndex]->pt,electrons[electronOneIndex]->eta, electrons[electronOneIndex]->phi,ELE_MASS);	
				tempLepTwoP4.SetPtEtaPhiM(electrons[j]->pt,electrons[j]->eta, electrons[j]->phi,ELE_MASS);
				tempDILEP = tempLepOneP4 + tempLepTwoP4;
				if(
					//tempDILEP.M() < 100 && tempDILEP.M() > 80
					electrons[j]->q == -electrons[electronOneIndex]->q
					&& tempDILEP.M() > 50
					//&& tempDILEP.M() < 110
				){
					hasValidPair = true;
					...
					leptonTwoP4.SetPtEtaPhiM(electrons[electronTwoIndex]->pt, electrons[electronTwoIndex]->eta, electrons[electronTwoIndex]->phi, ELE_MASS);
				}
			}

	       	 	if (!hasValidElectron || !hasValidPair)
	       		    return kTRUE;
		}
		eventStep++;
	        hTotalEvents->Fill(eventStep);
	...
	if(MuFlag){
		leptonOneP4.SetPtEtaPhiM(muons[muonOneIndex]->pt, muons[muonOneIndex]->eta, muons[muonOneIndex]->phi, MUON_MASS);
		leptonTwoP4.SetPtEtaPhiM(muons[muonTwoIndex]->pt, muons[muonTwoIndex]->eta, muons[muonTwoIndex]->phi, MUON_MASS);
		dileptonP4 = leptonOneP4 + leptonTwoP4;
		//if(dileptonP4.M() < 10)
		//	cout << dileptonP4.M() << endl;
	}
	else if(ElFlag){
		leptonOneP4.SetPtEtaPhiM(electrons[electronOneIndex]->pt, electrons[electronOneIndex]->eta, electrons[electronOneIndex]->phi, ELE_MASS);
		leptonTwoP4.SetPtEtaPhiM(electrons[electronTwoIndex]->pt, electrons[electronTwoIndex]->eta, electrons[electronTwoIndex]->phi, ELE_MASS);
	...
	*/
	...
	if(jets.size() > 1){
			...
			for (unsigned int j = i+1; j < jets.size(); ++j) {
				...
				TLorentzVector tempJJG = tempDiJet + photonOneP4;
	...
	*/
	if(debugFlag)
        ...
        for (unsigned int i = 0; i < genPhotons.size(); ++i) {
            ...
            //cout << "---Hard Process Final:: " <<  pho->fromHardProcessFinalState 
	    //	 << "---Prompt Final:: " << pho->isPromptFinalState << endl;
	...
	if(hasValidPhoton && hasValidPair){	
		////////////////////////////////////////////////////////////////////////////////
		leptonOnePt  = leptonOneP4.Pt();
		leptonOneEta = leptonOneP4.Eta();
		leptonOnePhi = leptonOneP4.Phi();
		leptonOneE   = leptonOneP4.Energy();
		leptonOneEt  = leptonOneP4.Et();
		leptonOneM   = leptonOneP4.M();

		leptonTwoPt  = leptonTwoP4.Pt();
		leptonTwoEta = leptonTwoP4.Eta();
		leptonTwoPhi = leptonTwoP4.Phi();
		leptonTwoE   = leptonTwoP4.Energy();
		leptonTwoEt  = leptonTwoP4.Et();
		leptonTwoM   = leptonTwoP4.M();
		
		photonOnePt  = photonOneP4.Pt();
		photonOneEta = photonOneP4.Eta();
		photonOnePhi = photonOneP4.Phi();
		photonOneE   = photonOneP4.Energy();
		photonOneEt  = photonOneP4.Et();
		photonOneM  = photonOneP4.M();
		//jetOneP4.SetPtEtaPhiM(jets[jetOneIndex]->pt, jets[jetOneIndex]->eta, jets[jetOneIndex]->phi, jets[jetOneIndex]->mass);
		//jetTwoP4.SetPtEtaPhiM(jets[jetTwoIndex]->pt, jets[jetTwoIndex]->eta, jets[jetTwoIndex]->phi, jets[jetTwoIndex]->mass);
		...
		//cout << "  " << (leptonOneP4 + leptonTwoP4).M() << "   " << endl;//cout << "never" << endl;
		dileptonPt   = (leptonOneP4 + leptonTwoP4).Pt();
		dileptonE    = (leptonOneP4 + leptonTwoP4).Energy();
		dileptonEta  = (leptonOneP4 + leptonTwoP4).Eta();
		dileptonPhi  = (leptonOneP4 + leptonTwoP4).Phi();
		dileptonM    = (leptonOneP4 + leptonTwoP4).M(); 
		//dileptonM    = dileptonP4.M();
		//dileptonPt   = dileptonP4.Pt();
		//dileptonE    = dileptonP4.Energy();
		...
		TLorentzVector dilepgmP4 = dileptonP4 + photonOneP4;
			
		dilepgmPt   = (leptonOneP4 + leptonTwoP4 + photonOneP4).Pt();
		dilepgmE    = (leptonOneP4 + leptonTwoP4 + photonOneP4).Energy();
		dilepgmEta  = (leptonOneP4 + leptonTwoP4 + photonOneP4).Eta();
		dilepgmPhi  = (leptonOneP4 + leptonTwoP4 + photonOneP4).Phi();
		dilepgmM    = (leptonOneP4 + leptonTwoP4 + photonOneP4).M();
		dilepgmDEta = fabs(dileptonP4.Eta() - photonOneP4.Eta());
		dilepgmDPhi = fabs(dileptonP4.DeltaPhi(photonOneP4));
		dilepgmDR   = dileptonP4.DeltaR(photonOneP4);
		//dilepgmPt   = dilepgmP4.Pt();
		//dilepgmE    = dilepgmP4.Energy();
		//dilepgmEta  = dilepgmP4.Eta();
		//dilepgmPhi  = dilepgmP4.Phi();
		//dilepgmM    = dilepgmP4.M();
		//dilepgmDEta = fabs(dileptonP4.Eta() - photonOneP4.Eta());
		//dilepgmDPhi = fabs(dileptonP4.DeltaPhi(photonOneP4));
		//dilepgmDR   = dileptonP4.DeltaR(photonOneP4);
	...
	*/
	...
	if(MuFlag){
		...
		if(!hasValidMuon) //|| hasValidPair)
			return kTRUE;
		eventStep++;
	        hTotalEvents->Fill(eventStep);
		leptonOneP4.SetPtEtaPhiM(muons[muonOneIndex]->pt, muons[muonOneIndex]->eta, muons[muonOneIndex]->phi, MUON_MASS);
		if(hasValidPair)  leptonTwoP4.SetPtEtaPhiM(muons[muonTwoIndex]->pt, muons[muonTwoIndex]->eta, muons[muonTwoIndex]->phi, MUON_MASS);
	}else if(ElFlag){
		//////////////
		// ELECTRON //
		//////////////
		...
		if(hasValidPair)  leptonTwoP4.SetPtEtaPhiM(electrons[electronTwoIndex]->pt, electrons[electronTwoIndex]->eta, electrons[electronTwoIndex]->phi, ELE_MASS);
	}
	if(debugFlag)
		cout << "--------muon pass\n";
	////////////
			...
			for (unsigned int j = i+1; j < jets.size(); ++j) {
				...
				TLorentzVector tempJJG = tempDiJet + photonOneP4;
					...
					&& tempJetTwo.DeltaR(photonOneP4) >= 0.4*/
					 //tempDiJet.M() >= 30  && tempDiJet.M() <= 120
					 tempDiJet.M() >= 50  && tempDiJet.M() <= 120
                			&& tempJJG.M() > 100. && tempJJG.M() < 140.
                			//&& tempJJG.M() > 80. && tempJJG.M() < 180.
   ...
   } // end eljetjetgm selection


    if(params->selection == "jetjetgmjetjet"){
	if(debugFlag){
		cout << "--------ENTER\n";
		cout << "||M:: " << nMuons << "|| EL:: "<< nElectrons << "|| T:: "<< nTaus << "|| Ph:: "<< nPhotons << "|| Jets:: "<< jets.size() << "||\n";
	}
