...
void zAnalyzer::Begin(TTree *tree)
{
 ...
 // Param Formating
    ...
    outTree->Branch("ProbeIDISOPass"        , &ProbeIDISOPass);
...
Bool_t zAnalyzer::Process(Long64_t entry)
{
    ...
    if (params->selection == "ee") {
		...
		for (unsigned int p = 0; p < genLeptons.size(); ++p) {
			TGenParticle *elec = genLeptons.at(p);
		...
		if( 	tempElec.Pt() > 30 && 
			...
			//for (int j = 0; j < fPhotonArr->GetEntries(); j++) {
			for (unsigned int j = 0; j < photons.size(); j++) {
			//	TPhoton* photon = (TPhoton*) fPhotonArr->At(j);
				...
				*/
				if(		
					...
					string phoSIEIE = "False"; 
					string phoHOVER = "False";
					if( fabs(photon->scEta) > 1.566 ){	
						if( photon->sieie < 0.045)
							phoSIEIE = "True";
						if( photon->hovere < 0.05)
							phoHOVER = "True";
					}
					else if( fabs(photon->scEta) < 1.479 ){
						if( photon->sieie < 0.015)
							phoSIEIE = "True";
						if( photon->hovere < 0.08)
							phoHOVER = "True";
					}

					cout << "+++++++++++++++++++++++++++++++++++\n";	
					cout << "---- Sieie  :: " << photon->sieie  << "  |  " << phoSIEIE << endl;
					cout << "---- Hovere :: " << photon->hovere << "  |  " << phoHOVER  << endl;

					if( 	particleSelector->PassPhotonID(photon, cuts->preSelPhID) )
						cout << "---- ID :: "<< "TRUE" << endl;
					else
						cout << "---- ID :: "<< "FALSE" << endl;
					if(!effISO){
						...
						if( 	particleSelector->PassPhotonID(photon, cuts->preSelPhID) ){
							...
							PhotonProbeIDPass->Fill(tempPhoton.Pt(),tempPhoton.Eta(),1);
							ProbeIDPass = true; 
							...
							PhotonProbeIDFail->Fill(tempPhoton.Pt(),tempPhoton.Eta(),1);
							ProbeIDPass = false;
						...
						if(     particleSelector->PassPhotonID (photon, cuts->preSelPhID) ){ 
							if(
								...
								PhotonProbeIDISOPass->Fill(tempPhoton.Pt(),tempPhoton.Eta(),1);
								ProbeIDISOPass = true;
								...
								PhotonProbeIDISOFail->Fill(tempPhoton.Pt(),tempPhoton.Eta(),1);
								ProbeIDISOPass = false;
				...
				//if(ProbeIDISOPass || ProbeIDPass)
				//	break;
			} // Probe loop
			//TElectron* ELEC = (TElectron*) fElectronArr->At(TagIDPassIndex  );
			//TPhoton*   PHOT = (TPhoton*  ) fPhotonArr  ->At(ProbeIDPassIndex);

			TElectron* ELEC = electrons[TagIDPassIndex  ];
			TPhoton*   PHOT = photons    [ProbeIDPassIndex];

			TLorentzVector Dilep, Photon, Electron;
			Electron.SetPtEtaPhiM(ELEC->calibPt, ELEC->eta, ELEC->phi, ELE_MASS);
			Photon  .SetPtEtaPhiM(PHOT->calibPt, PHOT->eta, PHOT->phi, ELE_MASS);
