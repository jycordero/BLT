...
Bool_t zgAnalyzer::Process(Long64_t entry)
{
	...
	cout << "=========================";
	cout << "Start ..........\n";
	cout 	<< "Period:: " << params->period << " Selection:: " << params->selection
		<< "DataSet:: " << params->dataset  << " DataSetGroup:: " << params->datasetgroup  
		<< endl;
    }
    ...
    for (int i=0; i<fPhotonArr->GetEntries(); i++) {
        ...
        if (sync_print_precut || debug) {
            cout << "photon_pt, photon_calibpt, photon_eta, photon_sc_eta, photon_phi, photon_mva16, photonmva17, pass_electron_veto" << endl;
            cout << photon->pt << ", " << photon->calibPt << ", " << photon->eta << ", " << photon->scEta << ", " << photon->phi << ", " << photon->mvaSpring16 << photon->mvaFall17V2 
    ...
    cout << "XXXXXXXXXXXXX Photon Size " << photons.size() << endl;
    if (params->selection == "mumug") {
        if (muons.size() < 2) 
            return kTRUE;
        hTotalEvents->Fill(5);
