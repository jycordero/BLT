...
void hzgAnalyzer::Begin(TTree *tree)
{
	...
	//Single BTag trigger
	triggerNames.push_back("HLT_QuadPFJet_BTagCSV_p016_VBF_Mqq460_v*");
	triggerNames.push_back("HLT_QuadPFJet_BTagCSV_p016_VBF_Mqq500_v*");

	//Double BTag trigger
	//triggerNames.push_back("HLT_QuadPFJet_BTagCSV_p016_p11_VBF_Mqq200_v*");
	//triggerNames.push_back("HLT_QuadPFJet_BTagCSV_p016_p11_VBF_Mqq240_v*");
    ...
    if(triggerNames[0] == "HLT_QuadPFJet_BTagCSV_p016_VBF_Mqq460_v*"){
	//PreSel_pt_leadjet = 15;
	//Sel_pt_leadjet	 = 10;	
	//PreSel_pt_trailjet = 10;
	//Sel_pt_trailjet	 = 5;
    }
    ...
    outTree->Branch("genJetFourP4", &genJetFourP4);
    outTree->Branch("genZBosonP4", &genZBosonP4);
    outTree->Branch("genHiggsP4", &genJetFourP4);
...
Bool_t hzgAnalyzer::Process(Long64_t entry)
{
    ...
    if(params->selection == "mugmjetjet"){
	    ...
	    if (!isData) {
			...
			if(particle->parent >0){	
				...
				if(fabs(particle->pdgId) == 13 && (fabs(mother->pdgId) == 23|| fabs(mother->pdgId) == 24)) LeptonMom = 1;
				if(fabs(particle->pdgId) == 13) nLepton++;
    ...
    else if(params->selection == "jetjetgmjetjet"){
	if (!isData) {
		vector<TGenParticle*> part;
		haveJet = false;
		haveAntiJet = false;
		fEvent = false;
		fPh = false;
		...
		unsigned count = 0;             
        	for (int i = 0; i < fGenParticleArr->GetEntries(); ++i) {      	
                	TGenParticle* particle = (TGenParticle*) fGenParticleArr->At(i);
                	//part.push_back((TGenParticle*) fGenParticleArr->At(i));
			//if(particle->parent >0){	
			if(particle->parent > 0){
				if(
					   particle->pdgId == 1
				...
				TGenParticle* mother = (TGenParticle*) fGenParticleArr->At(particle->parent);	
				//part.push_back((TGenParticle*) fGenParticleArr->At(part.back()->parent));
				
				//if( part[0]->pdgId == 22 and part[1]->pdgId == 25){
				if( particle->pdgId == 22 and mother->pdgId == 25){
					//genPhotonP4.SetPtEtaPhiM(part[0]->pt, part[0]->eta, part[0]->phi, part[0]->mass);	
					genPhotonP4.SetPtEtaPhiM(particle->pt, particle->eta, particle->phi, particle->mass);	
					fPh = true;							
					nPh++;
				}
				if(mother->parent > 0) {
				//if(part.back()->parent > 0) {
					TGenParticle* grandma = (TGenParticle*) fGenParticleArr->At(mother->parent);
					//part.push_back((TGenParticle*) fGenParticleArr->At(part.back()->parent));
					part.push_back((TGenParticle*) fGenParticleArr->At(grandma->parent));
					//cout << "Part: " << particle->pdgId << " | mom: " << mother->pdgId << " | grandma: " << grandma->pdgId << endl;
					//if(grandma->parent > 0) {
					if(part.back()->parent > 0) {
						for(int k = 0; k < 100;k++){
							if(
							...
							cout << k << " ";
							part.push_back((TGenParticle*) fGenParticleArr->At(part.back()->parent));
							if(part.back()->parent <= 0) break;	
						}
						TGenParticle* grand2ma = part.back();//(TGenParticle*) fGenParticleArr->At(grandma->parent);
						//cout << "~~part: " << part[0]->pdgId << " | mom: " << part[1]->pdgId << " | grandma: " << part[2]->pdgId << " | grand2ma : " << part[3]->pdgId << endl;
						if( (haveJet or haveAntiJet) and mother->pdgId == 23 and grandma->pdgId == 25){
						//if((haveJet or haveAntiJet) and part[1]->pdgId == 23 and part[2]->pdgId == 25){
							//cout << "~~part: " << part[0]->pdgId << " | mom: " << part[1]->pdgId << " | grandma: " << part[2]->pdgId << " | grand2ma : " << part[3]->pdgId << endl;
							cout << "~~Part: " << particle->pdgId << " | mom: " << mother->pdgId << " | grandma: " << grandma->pdgId << " | grand2ma : " << grand2ma->pdgId << endl;
							//cout << " Grand2ma : " << grand2ma->pdgId <<  " || Grand2ma : " << grand3ma->pdgId <<  " || Grand4ma : " << grand4ma->pdgId << endl;
							if(haveJet)     genJetOneP4.SetPtEtaPhiM(particle->pt, particle->eta, particle->phi, particle->mass);	
							if(haveAntiJet) genJetTwoP4.SetPtEtaPhiM(particle->pt, particle->eta, particle->phi, particle->mass);	
							genZBosonP4.SetPtEtaPhiM(mother->pt, mother->eta, mother->phi, mother->mass);	
							genHiggsP4.SetPtEtaPhiM(grandma->pt, grandma->eta, grandma->phi, grandma->mass);	
							//if(haveJet) genJetOneP4.SetPtEtaPhiM(part[0]->pt, part[0]->eta, part[0]->phi, part[0]->mass);	
							//if(haveAntiJet) genJetTwoP4.SetPtEtaPhiM(part[0]->pt, part[0]->eta, part[0]->phi, part[0]->mass);	
							//genZBosonP4.SetPtEtaPhiM(part[1]->pt, part[1]->eta, part[1]->phi, part[1]->mass);	
							//genHiggsP4.SetPtEtaPhiM(part[2]->pt, part[2]->eta, part[2]->phi, part[2]->mass);	
							fB = true;
							nB++;	
							if(
								   grand2ma->pdgId == 1
								...
								genJetThreeP4.SetPtEtaPhiM(grand2ma->pt, grand2ma->eta, grand2ma->phi, grand2ma->mass);	
								fVBF = true;
							}
							else if(
								   grand2ma->pdgId == -1
								...
								genJetFourP4.SetPtEtaPhiM(grand2ma->pt, grand2ma->eta, grand2ma->phi, grand2ma->mass);	
		...
		outBugTree->Fill();
		cout << "fVBF: " << fVBF << " || fPh: " << fPh << " || fB: " << fB << endl;
		flagGen = ( fVBF && fPh && fB);
		if(flagGen){
			//if(genPhotonP4.Pt() > Sel_pt_ph && genLeptonP4.Pt() > Sel_pt_mu1){
				outGenTree->Fill();
		...
		part.clear(); // clear vector of particles
		nPartons = count; // This is saved for reweighting inclusive DY and combining it with parton binned DY
    ...
    if(!noTrig){
	    for (unsigned i = 0; i < triggerNames.size(); ++i) {
	        ...
	        if (triggered) {
			//cout << "Triggered "<< triggerNames[i]<< endl;
