...
myROOTSYS2="/cvmfs/cms.cern.ch/slc6_amd64_gcc630/lcg/root/6.10.08-elfike/"
if [ "$ROOTSYS" != "$myROOTSYS" ] && [ "$ROOTSYS" != "$myROOTSYS2" ]
then
    echo "ROOT 6.10 not loaded"
else


 # Rename data trees and histograms     (ugh it got gross)
    for suffix in "mumu_2017"
    do
        for tag in B C D E F
        do
            if  hadd    "$suffix$tag".root    output_muon_2017"$tag"_*.root
            then
                if  rm  output_muon_2017"$tag"_*.root
                then
                    echo Deleted output_"$suffix$tag"_*.root.
                fi
            fi

            root.exe -q -b "../../renameTree.C(\""$suffix$tag".root\", \"tree_"$suffix$tag"\", \\
                                               \"tree_"$suffix"\")"
            ...
            if rootmv   "$suffix$tag"".root:TotalEvents_""$suffix$tag" \\
                        "$suffix$tag"".root:TotalEvents_""$suffix"
            then
...
#   mv  muon_2016H.root         muon_2016_7.root
    ...
    mv  mumu_2017B.root         mumu_2017_1.root
    mv  mumu_2017C.root         mumu_2017_2.root
    mv  mumu_2017D.root         mumu_2017_3.root
    mv  mumu_2017E.root         mumu_2017_4.root
    mv  mumu_2017F.root         mumu_2017_5.root
...
#   fi
 ...
 #  DY M-50
    if  hadd zjets_m-50_1.root  output_DYJetsToLL_M-50_?.root   && \
        hadd zjets_m-50_2.root  output_DYJetsToLL_M-50_1?.root  && \
        hadd zjets_m-50_3.root  output_DYJetsToLL_M-50_2?.root
    then
...
#       hadd z2jets_m-50_2.root output_DY2JetsToLL_M-50_3.root  output_DY2JetsToLL_M-50_4.root  \
#                               output_DY2JetsToLL_M-50_5.root  output_DY2JetsToLL_M-50_6.root  \
#                               output_DY2JetsToLL_M-50_7.root  output_DY2JetsToLL_M-50_8.root  \
#                               output_DY2JetsToLL_M-50_9.root
#   then
...
#   fi
 ...
 # Move to EOS
#   for suffix in   "electron_2016" "muon_2016" "dy_m-10to50" "dy_m-50" "z1jets_m-10to50"         \
#                   "z1jets_m-50" "z2jets_m-10to50" "z2jets_m-50" "z3jets_m-10to50" "z3jets_m-50" \
#                   "z4jets_m-10to50" "z4jets_m-50" "ggH_zz_4l" "H_zz_4l" "ttbar" "ttz_2l2nu"     \
#                   "ww_2l2nu" "wz_3lnu" "zz_4l"
    for suffix in "mumu_2017" "zjets_m-50"
    do
        eos root://cmseos.fnal.gov mkdir -p /store/user/jrainbol/Trees/2017_test/"$suffix"
        xrdcp   -f  "$suffix"_*.root    \
                root://cmseos.fnal.gov//store/user/jrainbol/Trees/2017_test/"$suffix"/.
    done

    xrdcp   -f  source.tar.gz   root://cmseos.fnal.gov//store/user/jrainbol/Trees/2017_test/.
